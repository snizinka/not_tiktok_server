...
class GetUnreadPrivateMessages extends Message {
    // TODO: Not written query
    static async getMessages(userId) {
        let messages = await query("SELECT msgs.`chat-messagesId` as messageId, msgs.seen, msgs.contactId, msgs.message, msgs.deliveryTime, msgs.authorId, msgs.forwarded_from, msgs.forward_chat " +
            ...
            "WHERE msgs.authorId != " + userId + " AND (msgs.seen = 0 OR msgs.seen IS NULL) order by msgs.`chat-messagesId` desc")
...
module.exports = GetUnreadPrivateMessages