...
class GetLastPrivateMessagesByContactId extends Message {
    static async getMessages(contactId) {
        let messages = await query("SELECT msgs.`chat-messagesId` as messageId, msgs.seen, msgs.contactId, msgs.message, msgs.deliveryTime, msgs.authorId, msgs.forwarded_from, msgs.forward_chat " +
            ...
            "WHERE msgs.contactId = " + contactId.contactId + " order by msgs.`chat-messagesId` desc limit 10")
        ...
        let unread = await GetUnreadPrivateMessagesById.getMessages({
            contactId,
            lastId: messageList.length > 0 ? messageList[messageList.length - 1].messageId : 0
        })
        
        messageList.push(...unread)
        return messageList
    }
}
